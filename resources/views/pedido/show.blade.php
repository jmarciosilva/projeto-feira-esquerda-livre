...
<main class="max-w-3xl mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold mb-1" style="color:#3D3000;">Pedido recebido!</h1>
        <p class="text-sm text-gray-500">Referência <strong>#{{ $order->reference }}</strong> · {{ $order->status->label() }}</p>
    </div>
    ...
    <div class="p-4 rounded-xl text-sm mb-8" style="background:#FDF8DC; color:#5C4500;">
        ...
        e envie o comprovante pelo WhatsApp informado. A loja confirmará o recebimento no painel dela.
        <br>{{ $order->delivery_type->value === 'retirada' ? '📦' : '🚚' }} <strong>{{ $order->delivery_type->label() }}:</strong> {{ $order->shipping_note }}
    </div>
    ...
    <div class="space-y-6 mb-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <a href="https://wa.me/55{{ preg_replace('/\D/', '', $expositor->whatsapp) }}?text={{ rawurlencode('Olá! Acabei de fazer o pedido #' . $order->reference . ' na Feira Esquerda Livre.') }}"
               target="_blank"
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 mb-8">
        ...
        <p class="text-sm text-gray-600">Combine o local e horário diretamente com o(s) lojista(s) pelo WhatsApp.</p>
        @else
        <h2 class="font-bold text-base mb-3" style="color:#3D3000;">🚚 Endereço de entrega</h2>
